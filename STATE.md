...
| **Action Economy** | Action, Bonus Action, Reaction trackers (click to spend/restore). Auto-clears reaction on new combat round. Auto-detects reaction use via `dnd5e.useItem` and `dnd5e.rollAttack` hooks. |
...
- [ ] **Refactor weapon stats** — `_weaponStats()` and `_showItemDescription()` share ~80 lines of duplicate attack bonus logic
